...
class CardsAdapter (var list: ArrayList<SelectedCreditCard>) : RecyclerView.Adapter<CardsAdapter.CreditCardHolder>() {
    var items =  list
    lateinit var listener : ((position: Int) -> Unit)
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CreditCardHolder =
        CreditCardHolder(LayoutInflater.from(parent.context).inflate(R.layout.card_item, parent, false))
    ...
    fun updateList(list : ArrayList<SelectedCreditCard>) {
        items = list
        notifyDataSetChanged()
    }

    inner class CreditCardHolder(view: View) : RecyclerView.ViewHolder(view) {
