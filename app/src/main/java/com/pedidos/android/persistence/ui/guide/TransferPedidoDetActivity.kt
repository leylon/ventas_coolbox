...
import kotlinx.android.synthetic.main.transfer_detail_activity.*
...
class TransferPedidoDetActivity : MenuActivity(), InventaryStatusPopUpFragment.newDialoglistenerAuxGuide {
    ...
    private lateinit var guideAdapter: TransferDetailAdapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentViewWithMenu(R.layout.transfer_detail_activity)
        ...
        guideAdapter = TransferDetailAdapter(mutableListOf()) { saleSubItem ->
    ...
    private fun addItem(productEntity: ProductEntity) {
        ...
        (rvwProducts.adapter as TransferDetailAdapter).deleteItems(items)
        (rvwProducts.adapter as TransferDetailAdapter).addItems(items)
        showProgress(false)
    ...
    private fun subscribeToModel(viewModel: GuideViewModel) {
        viewModel.saleLiveData.observe(this, Observer { newItem ->
            showProgress(false)
            if (newItem != null) {
                //(rvwProducts.adapter as _root_ide_package_.com.pedidos.android.persistence.ui.guide.TransferDetailAdapter).items.removeAll { true }
                (rvwProducts.adapter as TransferDetailAdapter).addItems(newItem.productos)
    ...
    fun actualizarInventario(entity: ProductEntity ){
        ...
        if (!bandera){
            AlertDialog.Builder(this, R.style.AppTheme_DIALOG)
                .setTitle(R.string.app_name)
                .setMessage("¿Estás seguro que deseas Añadir?")
    ...
    companion object {
        ...
        val TAG = TransferPedidoDetActivity::class.java.simpleName!!
    }
